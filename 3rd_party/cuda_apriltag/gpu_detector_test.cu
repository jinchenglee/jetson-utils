...
class GpuDetectorTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    // Read in the image
    bgr_img = cv::imread("../3rd_party/cuda_apriltag/data/colorimage.jpg", cv::IMREAD_COLOR);
    if (bgr_img.empty()) {
      FAIL() << "Could not read test image";
    }
    cvtColor(bgr_img, yuyv_img, cv::COLOR_BGR2YUV_YUYV);

    bgr_img_notags =
        cv::imread("../3rd_party/cuda_apriltag/data/colorimage_notags.jpg", cv::IMREAD_COLOR);
    if (bgr_img_notags.empty()) {
      FAIL() << "Could not read test image without tags";
    }
    cvtColor(bgr_img_notags, yuyv_img_notags, cv::COLOR_BGR2YUV_YUYV);
